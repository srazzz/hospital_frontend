export const postDoctorData = (
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const response = await fetch('http://192.168.0.183:3001/doctors', {
        method: 'POST',
        headers: {
...
export const postPatientData = (name, age, doctorId) => {
  ...
  console.log('http://192.168.0.183:3001/patients');
  return new Promise(async (resolve, reject) => {
    try {
      const response = await fetch('http://192.168.0.183:3001/patients', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
